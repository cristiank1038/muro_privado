...
CREATE TABLE IF NOT EXISTS `muro_privado`.`users` (
  ...
  `updted_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `muro_privado`.`messages` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_messges_users_idx` (`sender_id` ASC) VISIBLE,
  INDEX `fk_messges_users1_idx` (`receiver_id` ASC) VISIBLE,
  CONSTRAINT `fk_messges_users`
    FOREIGN KEY (`sender_id`)
    REFERENCES `muro_privado`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_messges_users1`
